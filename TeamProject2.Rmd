...
![](images/pexels-cottonbro-5077420.jpg){width="333"}

## Step 0: Why?

### Context, Objective, and Financial Model
...
context_data <- data.frame(
  ...
  Value = c("~640 Million", 
            "~252 Million (39% of MAU)", 
            "4 billion (85% through Premium Subscribers)",
            "~$13.5 / month", 
            "~2.0%", 
            "~$675"),
  Rationale = c("Based on publicly reported figures.", 
                ...
                "($4 billion * 0.85)/252 million", 
                "Industry estimate for major streaming services.", 
                "Calculated as ARPU / Churn Rate = $13.5 / 0.02.")
)
...
Further, there are 30 songs in the Discover weekly playlist. We assume that 20% of those are already hidden-gems.

### TP:

True positives have the following impact. Adding a hidden gem to the playlist results in an 80% chance of replacing a non–hidden gem and a 20% chance of replacing an existing hidden gem. Replacing a non–hidden gem increases the hidden-gem count from 6 to 7, a 16.67% lift. Because every 10% increase in hidden gems reduces churn by 0.01%, this lift translates to a 0.01667% churn reduction. The associated revenue impact is:

$$0.8 * \$ 252 \text{ million } * \$ 13.5 * 0.0001667$$

If the swap hits an existing hidden gem (20% probability), the hidden-gem count does not increase, creating an equivalent opportunity cost:

$$0.2 * \$ 252 \text{ million } * \$ 13.5 * 0.0001667$$

In total we have the following result: $$\$ 252 \text{ million } * \$ 13.5 * 0.0001667 * (0.8 - 0.2) = \$ 340,268.04  $$

### FP:

Adding a non–hidden gem has the worst consequences. With an 80% chance, it simply replaces another non–hidden gem already in the playlist, generating no benefit. The only impact is the missed opportunity of adding a hidden gem, which we capture as opportunity cost:

$$ 0.8 * \$ 252 \text{ million } * \$ 13.5 * 0.0001667 * (0.8 - 0.2) = 0.8 *  \$ 340,268.04 $$

There is, however, a 20% chance that the swap replaces a hidden gem. This reduces the hidden-gem count from 7 to 6 — a 16.67% decline — which increases the churn rate by 0.01667%. In this case, we incur both the revenue loss from higher churn and the missed opportunity of improving churn:

$$ 0.2 * (\$ 252 \text{ million } * 13.5 * (-0.0001667) - \$ 340,268.04 ) $$

Combining both outcomes gives the total value of a false positive:

$$ 0.2 * (\$ 252 \text{ million } * 13.5 * (-0.0001667) - \$ 340,268.04 ) - 0.8 *  \$ 340,268.04 = - \$ 453,690.72 $$

### FN:

For false negatives, the only impact is the opportunity cost of not adding a hidden gem:
...
financial_data <- data.frame(
  ...
  Financial_Value = c("+$340,268.04 ", "-$453,690.72", "-$340,268.04"),
  Financial_Rationale = c("Revenue: Estimated net monthly value derived from the 0.01667% reduction in churn for a high-value, retained customer. (Maximize)", "Cost: Significant loss from removing high-engaging content, and opportunity cost of promoting non-engaging content that accelerates churn. (Minimize - Critical Error)", "Cost: Loss from missing a viable track and losing the opportunity for increased engagement and retention. (Minimize)")
)
...
$$
\text{Net Impact} = \frac{(\text{TP} \times \$340,268.04) - (\text{FP} \times \$453,690.72) - (\text{FN} \times \$340,268.04)}{\text{# Months of Songs}} 
$$
...
First, looking at our individual Level 1 models, the Random Forest appears to be the best-performing model with the highest overall accuracy of 80.42% and highest Kappa of 0.43. Its strong specificity allowed it to effectively avoid False Positives, which we established to be extremely expensive in this financial context, as Spotify’s recommendation engine would be promoting non-engaging content and thus face a large opportunity cost. However, it is important to note that it has a relatively lower sensitivity (49.38%), meaning that it still fails to identify many songs that would have driven higher engagement and reduced churn. While the Logistic Regression and ANN models also performed relatively well with better balance between sensitivity and accuracy, neither surpassed the Random Forest in terms of financial performance.

Now, comparing our best Level 1 model to our Level 2 model, we see that this stacked model has the highest net financial return and the least projected loss (\$1,007,537), representing an 87.1% improvement over the baseline. While improvements in accuracy and Kappa compared to the best Level 1 models were incremental, this model is better able to balance risk by combining linear and non-linear learning patterns while intelligently optimizing threshold decisions to minimize the most expensive prediction errors. In a business environment where even a 0.01% churn reduction represents millions of dollars in preserved Customer Lifetime Value, financially guided optimization is essential. Thus, out of all the models created, this stacked model would serve as the best decision engine for automated playlist curation like Discover Weekly.

![](images/pexels-yente-van-eynde-1263034-2403054.jpg){width="333"}

### Possible Errors

There are several limitations with the work we did that are worth noting. First, the music industry has rapidly shifting trends that may not be accounted for in this data set and may not hold over time. Additionally, we defined popularity based on the popularity metric given in the data set, which is an abstraction; we do not know the directly measured engagement outcomes that went into creating this metric (such as skip rate, replay frequency, playlist add-rate, etc.), meaning that the true use value is ambiguous. Additionally, the features we accounted for were mostly limited to audio characteristics and genre information, excluding information that is more contextual such as listener history or situational preferences. Finally, popularity is heavily influenced by artist fame. A song released by a globally recognized artist like Taylor Swift will likely have a high popularity score regardless of its audio characteristics, a bias that our model is not explicitly handling.
